...
export class ActorRdfMetadataExtractHydraControls extends ActorRdfMetadataExtract {
   ...
   */
  public getLinks(pageUrl: string, hydraProperties: {[property: string]: {[subject: string]: string[]}}): {[id: string]: any} {
    ...
    if (searchFormData) {
      for (const dataset in searchFormData) {
        for (const searchFormId of searchFormData[dataset]) {
          ...
          const getUri = (entries: {[id: string]: string}) => searchTemplate.expand(Object.keys(entries).reduce((variables: {[id: string]: string}, key) => {
