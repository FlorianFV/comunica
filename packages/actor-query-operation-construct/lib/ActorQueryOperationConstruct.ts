...
export class ActorQueryOperationConstruct extends ActorQueryOperationTypedMediated<Algebra.Construct> {
  ...
  public async runOperation(pattern: Algebra.Construct, context: ActionContext): Promise<IActorQueryOperationOutputQuads> {
    ...
    if (output.metadata) {
      metadata = () => (<() => Promise<{[id: string]: any}>> output.metadata)().then(m => {
        if (m.totalItems) {
          return { ...m, totalItems: m.totalItems * pattern.template.length };
        }
        return m;
      });
    }
