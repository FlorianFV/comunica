...
export class ActorRdfDereferencePagedNext extends ActorRdfDereferencePaged implements IActorRdfDereferencePaged {
   ...
   */
  protected async cloneOutput(outputPromise: Promise<IActorRdfDereferencePagedOutput>): Promise<IActorRdfDereferencePagedOutput> {
  ...
  protected async runAsync(action: IActionRdfDereferencePaged): Promise<IActorRdfDereferencePagedOutput> {
    ...
    const firstPageMetadata: () => Promise<{[id: string]: any}> = () => materializedFirstPageMetadata || (materializedFirstPageMetadata = this.mediatorMetadataExtract.mediate(
      { context: action.context, url: firstPageUrl, metadata: firstPageMetaSplit.metadata },
    )
      .then(output => output.metadata));

    const data: MediatedPagedAsyncRdfIterator = new MediatedPagedAsyncRdfIterator(firstPageUrl, firstPageMetaSplit.data, firstPageMetadata, this.mediatorRdfDereference, this.mediatorMetadata, this.mediatorMetadataExtract, action.context);
